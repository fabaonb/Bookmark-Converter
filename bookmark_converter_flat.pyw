...
def convert_chrome_json_no_shell(data):
    lines = []
    ...
    def process_item(item, indent='    '):
        if item.get('type') == 'url':
            ...
            lines.append(f'{indent}<DT><H3>{item.get("name", "")}</H3>')
            lines.append(f'{indent}<DL><p>')
            for child in item.get('children', []):
                process_item(child, indent + '    ')
            lines.append(f'{indent}</DL><p>')
    
    roots = data.get('roots', {})
    for root_key in ['bookmark_bar', 'other', 'synced']:
        if root_key in roots:
            root_folder = roots[root_key]
            for item in root_folder.get('children', []):
                process_item(item)
    
    return '\n'.join(lines)

def convert_firefox_sqlite_no_shell(db_path):
    lines = []
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    
    def process_item(parent_id, indent='    '):
        ...
        for item in items:
            ...
            if bm_type == 1:
                ...
                lines.append(f'{indent}<DL><p>')
                process_item(bm_id, indent + '    ')
                lines.append(f'{indent}</DL><p>')
    
    cursor.execute("SELECT id, title FROM moz_bookmarks WHERE type = 2 AND parent = 0")
    root_folders = cursor.fetchall()
    
    for root_id, root_title in root_folders:
        cursor.execute("SELECT id, title, type FROM moz_bookmarks WHERE parent = ?", (root_id,))
        subfolders = cursor.fetchall()
        for subfolder in subfolders:
            sub_id, sub_title, sub_type = subfolder
            if sub_type == 2:
                process_item(sub_id)
...
def detect_and_convert(file_path):
    ...
    try:
        if ext == '.json' or ext == '':
            ...
            if 'roots' in data:
                return convert_chrome_json_no_shell(data)
        elif ext in ['.sqlite', '.db']:
            return convert_firefox_sqlite_no_shell(file_path)
...
def main():
    ...
    for file_path in files:
        ...
        if content:
            output_path = os.path.splitext(file_path)[0] + '_flat.html'
